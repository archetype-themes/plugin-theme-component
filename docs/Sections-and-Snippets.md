...
Archie Components are either Sections or Snippets. Their structure is identical, but they are used in different ways.
...
need all the extra fluffy stuff. Outside of `package.json`, The only mandatory file is the `main.liquid` file. It needs
to be inside a `src` folder. The `src` folder is mandatory. The `index.js` and `main.css` files are optional.

```shell
.
├── README.md                             [ Optioanl Component README file ]
├── package.json                          [ Mandatory NodeJS Package file ]
├── build                                 [ Generated by Archie upon Section Build ]
└── src                                   [ Mandatory Sources Folder ]
...
**package.json**
Your standard Node.js package configuration file, pre-populated with a few presets to help Archie with component type and
the likes.
