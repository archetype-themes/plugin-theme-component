...
describe('theme component copy', () => {
  ...
  it('adds copyright information to copied files', async () => {
    ...
    const cssContent = fs.readFileSync(path.join(testThemePath, 'assets', 'to-be-copied.css'), 'utf8')
    const year = new Date().getFullYear()
    const cssExpectedHeader = `/* @archetype-themes/test-collection v1.0.1 | Copyright © ${year} Test Author | MIT */`
    expect(cssContent.startsWith(cssExpectedHeader)).to.be.true

    const jsContent = fs.readFileSync(path.join(testThemePath, 'assets', 'to-be-copied.js'), 'utf8')
    const jsExpectedHeader = `// @archetype-themes/test-collection v1.0.1 | Copyright © ${year} Test Author | MIT`
    expect(jsContent.startsWith(jsExpectedHeader)).to.be.true

    const liquidContent = fs.readFileSync(path.join(testThemePath, 'snippets', 'to-be-copied.liquid'), 'utf8')
    const liquidExpectedHeader = `{% # @archetype-themes/test-collection v1.0.1 | Copyright © ${year} Test Author | MIT %}`
    expect(liquidContent.startsWith(liquidExpectedHeader)).to.be.true
  })
})
