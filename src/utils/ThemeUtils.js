...
class ThemeUtils {
  ...
  static async findCollectionPackageRootFolder (collectionName) {
    const childRepoPath = join(dirname(env.npm_package_json), 'node_modules', ArchieNodeConfig.DEFAULT_PACKAGE_SCOPE, collectionName)
    if (await FileUtils.isReadable(childRepoPath))
      return childRepoPath
    else {
      const parentRepoPath = join(env.PROJECT_CWD, 'node_modules', ArchieNodeConfig.DEFAULT_PACKAGE_SCOPE, collectionName)
